...
class KMeans:
    def __init__(
      self, n_clusters=20, init='mn_2sg_rng', n_init=15, max_iter=300):
...
while (i >= 0):
    ...
    while(j < columns):
        if(raw_data[i][j] == "1"):
            Data.append([j+1,abs(i+1 - columns)])
        j = j + 1
    i = i - 1
KNN_A = Data
...
for i in range(0,clusters):
    ...
    for j in range(0,len(grps[i]['points'])):
        dist = math.sqrt((grps[i]['points'][j][0] - centroid_x)**2 + (grps[i]['points'][j][1] - centroid_y)**2)
        if(dist < min_dist):
            x_val = grps[i]['points'][j][0]
            y_val = grps[i]['points'][j][1]
            min_dist = dist
