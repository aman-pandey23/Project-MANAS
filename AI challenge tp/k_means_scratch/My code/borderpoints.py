...
class KMeans:
    def __init__(
      self, n_clusters=20, init='mn_2sg_rng', n_init=15, max_iter=1000):
...
def area(x,y,z):
    area = (1/2)*(x[0]*(y[1] - z[1]) + y[0]*(z[1] - x[1]) + z[0]*(x[1] - y[1]))
    return area

def remove_collinear(pointset):
    print(pointset)
    a = []
    ind = len(pointset) - 2
    if(ind > 0):
        for i in range(0,ind):
            if(area(pointset[i],pointset[i+1],pointset[i+2]) == 0):
                a.append(pointset[i+1])
    return a
...
for i in range(0,clusters):
    ...
    y1 = []
    points = []
    for j in range(0,len(grps[i]['points'])):
        x1.append(grps[i]['points'][j][0])
        y1.append(grps[i]['points'][j][1])
        points.append(Point(grps[i]['points'][j][0],grps[i]['points'][j][1]))
    a = convexHull(points, len(points))
    to_rem = remove_collinear(a)
    edge_points = list_diff(a,to_rem)
    for k in range(0,len(edge_points)):
        if(k < (len(edge_points)-1)):
            line = Line(edge_points[k],edge_points[k+1])
            for l in grps[i]['points']:
                if(line.onLine(l)):
                    points_out.append(l)
        else:
            line = Line(edge_points[k],edge_points[0])
            for l in grps[i]['points']:
                if(line.onLine(l)):
                    points_out.append(l) 
...
points_out = list_diff(points_out,edg)
for i in edg:
    points_out.append(i)


sorted_points_out = sorted(points_out, key=lambda x:(x[0], x[1]))
for i in sorted_points_out:
    print(i[0],i[1])
